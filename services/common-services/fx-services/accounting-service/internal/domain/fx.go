...
type Currency struct {
	...
	DemoInitialBalance   int64     `json:"demo_initial_balance" db:"demo_initial_balance"` // In smallest unit
	MinAmount            int64     `json:"min_amount" db:"min_amount"`                     // Minimum transaction amount
	MaxAmount            *int64    `json:"max_amount,omitempty" db:"max_amount"`           // Maximum transaction amount (NULL = unlimited)
	...
	return []*Currency{
		{
			...
			IsActive:           true,
			DemoEnabled:        true,
			DemoInitialBalance: 1000000, // $10,000.00 (in cents)
			MinAmount:          1,       // $0.01
		...
		},
		{
			...
			Name:               "Tether USD",
			Symbol:             &usdtSymbol,
			Decimals:           2, // Match schema (2 decimals, not 6)
			IsFiat:             false,
			IsActive:           true,
			DemoEnabled:        true,
			DemoInitialBalance: 1000000, // 10,000 USDT (in cents)
			MinAmount:          1,       // 0.01 USDT
			MaxAmount:          nil,
			CreatedAt:          now,
			UpdatedAt:          now,
		},
		{
			...
			IsActive:           true,
			DemoEnabled:        true,
			DemoInitialBalance: 10000000, // 0.1 BTC (in satoshis)
...
func (c *Currency) WithinLimits(amount int64) bool {
