...
type Balance struct {
	...
	Balance          int64      `json:"balance" db:"balance"`                      // Total balance in smallest currency unit
	AvailableBalance int64      `json:"available_balance" db:"available_balance"`  // Available for withdrawal/trading
	PendingDebit     int64      `json:"pending_debit" db:"pending_debit"`          // Pending outgoing funds
	PendingCredit    int64      `json:"pending_credit" db:"pending_credit"`        // Pending incoming funds
...
type BalanceSnapshot struct {
	...
	Balance          int64     `json:"balance"`           // Total balance at this time
	AvailableBalance int64     `json:"available_balance"` // Available balance at this time
...
func (h *BalanceHistory) AddSnapshot(snapshot *BalanceSnapshot) {
	...
	if len(h.Snapshots) == 1 {
		...
		h.AvgBalance = total / int64(len(h.Snapshots))
	}
}
...
func (h *BalanceHistory) GetBalanceChange() int64 {
