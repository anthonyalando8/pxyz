...
func SetupRoutes(
	...
	// ============================================================
	// Group everything under /partner/auth
	// ============================================================
	r.Route("/partner/auth", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			r.Use(auth.Middleware) // just validate token if present
		...
		r.Group(func(pr chi.Router) {
			...
			// Serve uploads
			pr.Handle("/uploads/*", http.StripPrefix("/partner/auth/uploads/", http.FileServer(http.Dir(uploadDir))))
			// WebSocket
